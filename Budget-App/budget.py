class Category:
  ...
  def transfer(self,amount, new_category):
    if self.withdraw(amount, "Transfer to {}".format(new_category.description)):
      new_category.deposit(amount, "Transfer from {}".format(self.description))
      return True
    else:
      return False
  ...
  def __repr__(self):
    ...
    total="{:.2f}".format(self.__balance)
    return header + ledger + "Total: " + total
